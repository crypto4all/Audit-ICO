...
function contractFromAddress(address) {
  // Il me semble plus web3.eth.contract(kycOracleAbi).at(address);
  return new web3.eth.Contract(kycOracleAbi, address)
}
// currentICO ?? c'est quoi comme variable ? quel type ? où est elle initialisée ?
function kycClearAddress(currentIco, address, onTxHash) {
  return web3.eth.getTransactionCount(currentIco.oracleAddress).then(txCount => {
    return new Tx({
      ...
      // ils se basent sur quoi pour avoir ces informations ?? ico_config.json ??
      to: currentIco.smartContractAddress,
      from: currentIco.oracleAddress,
      value: web3.utils.toHex("0"),
      // Normalement c'est plus myContractInstance.myMethod(param1 [, param2, ...] [, transactionObject] [, defaultBlock] [, callback]);
      // en se basant sur web3js  https://github.com/ethereum/wiki/wiki/JavaScript-API#contract-methods
